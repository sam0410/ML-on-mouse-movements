...
for fileName in FileList:
	...
	for i in range (1,rows):
		...
		meanx=	meanx+(data[i,4].astype(np.float))/rows ;
		meany=	meany+(data[i,5].astype(np.float))/rows ;
	...
	features=np.array([avgTime,avgSpeed,TimeSpent,avgMoves,meanx,meany])
